...
export const aiCache: Record<string, any> = {};
...
// Core unified AI handler for /api/ai/:module
  ...
  try {
    ...
    switch (module) {
      ...
      case "report":
        isJson = true;
        model = "llama-3.3-70b-versatile";
        const reportType = (context as any).type || "executive";
...
export async function handleAnalyze(restaurantData: any, res: any) {
  try {
    const groq = getGroq();
    const completion = await groq.chat.completions.create({
      messages: [
...
// Handle /api/exa/search endpoint
export async function handleExaSearch(query: string, category: string, res: any) {
  try {
